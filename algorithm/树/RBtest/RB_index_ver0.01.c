...
int main()
{
    ...
    for (int set = 0; set < set_n; set ++){
        ...
        for (int i = 0; i < tree_size; i ++){
            ...
            RBT_NODE new = RBT_NODE_creat(unordered + i);
            RBT_insert(tree, new, RBT_cmp);
        }
        ...
        for (int i = 0; i < delete_seg; i ++){
            RBT_NODE del = RBT_search(tree, test_set + i, RBT_cmp);
            RBT_delete(tree, del);
        }

        for (int i = delete_seg; i < tree_size; i ++){
            if (RBT_get_index(tree, test_set + i, RBT_cmp) != i - delete_seg && test_set[i] != test_set[i - 1]){
                printf("error occured %d %d\n", test_set[i], test_set[i + 1]);
                printf("%d %d\n", RBT_get_index(tree, test_set + i, RBT_cmp), RBT_get_index(tree, test_set + i + 1, RBT_cmp));
